...
import {
  ...
  resolveParamValue, resolveAnimatedParams,
  type Keyframe, type ParamKeyframe, type ParamTracks,
} from './keyframes';
...
describe('clampKeyframeTime', () => {
  ...
  it('accepts the widened { id, time } shape directly, not just full Keyframe objects', () => {
    // Type-level regression check: clampKeyframeTime's first param widened
    // from Keyframe[] to { id: string; time: number }[] so ParamKeyframe[]
    // (which has no `params` field) satisfies it too, with no behavior
    // change to this existing call pattern.
    const bare: { id: string; time: number }[] = [{ id: 'a', time: 2 }, { id: 'b', time: 5 }, { id: 'c', time: 8 }];
    expect(clampKeyframeTime(bare, 'b', 6, 10)).toBe(6);
    expect(clampKeyframeTime(bare, 'b', 2.02, 10)).toBeCloseTo(2.05, 5);
  });
});

function pkf(id: string, time: number, value: number | string, curve: 'hold' | 'linear' = 'hold'): ParamKeyframe {
  return { id, time, value, curve };
}

describe('resolveParamValue', () => {
  it('returns the fallback when there is no track', () => {
    expect(resolveParamValue(undefined, 5, 42)).toBe(42);
    expect(resolveParamValue([], 5, 42)).toBe(42);
  });

  it('returns the single keyframe value regardless of time', () => {
    const track = [pkf('a', 5, 10)];
    expect(resolveParamValue(track, 0, 999)).toBe(10);
    expect(resolveParamValue(track, 999, 999)).toBe(10);
  });

  it('holds the first keyframe before the track starts', () => {
    const track = [pkf('a', 5, 10), pkf('b', 10, 20)];
    expect(resolveParamValue(track, 0, -1)).toBe(10);
  });

  it('holds the last keyframe after the track ends', () => {
    const track = [pkf('a', 5, 10), pkf('b', 10, 20)];
    expect(resolveParamValue(track, 100, -1)).toBe(20);
  });

  it('holds a numeric value across a hold segment, not a blend', () => {
    const track = [pkf('a', 0, 10, 'hold'), pkf('b', 10, 30)];
    expect(resolveParamValue(track, 5, -1)).toBe(10);
  });

  it('switches exactly at the next keyframe\'s own time, not before, on a hold segment', () => {
    const track = [pkf('a', 0, 10, 'hold'), pkf('b', 10, 30)];
    expect(resolveParamValue(track, 9.999, -1)).toBe(10);
    expect(resolveParamValue(track, 10, -1)).toBe(30);
  });

  it('linearly interpolates a numeric segment at the midpoint', () => {
    const track = [pkf('a', 0, 10, 'linear'), pkf('b', 10, 30)];
    expect(resolveParamValue(track, 5, -1)).toBe(20);
    expect(resolveParamValue(track, 2.5, -1)).toBe(15);
  });

  it('linearly interpolates a hex color segment channel-wise at the midpoint', () => {
    const track = [pkf('a', 0, '#000000', 'linear'), pkf('b', 10, '#FFFFFF')];
    expect(resolveParamValue(track, 5, '#000000')).toBe('#808080');
  });

  it('falls back to holding when a curve is linear but values are not both numbers/colors (defensive)', () => {
    const track = [pkf('a', 0, 'BOX_INVERT', 'linear'), pkf('b', 10, 'GHOST_TRAIL')];
    expect(resolveParamValue(track, 5, 'x')).toBe('BOX_INVERT');
  });

  it('holds a discrete (renderMode-style) segment', () => {
    const track = [pkf('a', 0, 'BOX_INVERT', 'hold'), pkf('b', 10, 'GHOST_TRAIL')];
    expect(resolveParamValue(track, 5, 'x')).toBe('BOX_INVERT');
    expect(resolveParamValue(track, 10, 'x')).toBe('GHOST_TRAIL');
  });

  it('holds across the most recent of three or more keyframes', () => {
    const track = [pkf('a', 0, 10), pkf('b', 5, 20), pkf('c', 10, 30)];
    expect(resolveParamValue(track, 2, -1)).toBe(10);
    expect(resolveParamValue(track, 5, -1)).toBe(20);
    expect(resolveParamValue(track, 7, -1)).toBe(20);
    expect(resolveParamValue(track, 10, -1)).toBe(30);
  });

  it('sorts out-of-order track entries by time before resolving', () => {
    const track = [pkf('b', 10, 30), pkf('a', 0, 10, 'linear')];
    expect(resolveParamValue(track, 5, -1)).toBe(20);
  });
});

describe('resolveAnimatedParams', () => {
  it('leaves every non-animatable field exactly as in base, and animatable fields with no track alone too', () => {
    const result = resolveAnimatedParams({}, 5, baseParams);
    expect(result).toEqual(baseParams);
  });

  it('falls back to base[key] for an animatable field with an empty track array', () => {
    const tracks: ParamTracks = { brightness: [] };
    const result = resolveAnimatedParams(tracks, 5, baseParams);
    expect(result.brightness).toBe(baseParams.brightness);
  });

  it('overrides only the animatable fields that have their own non-empty track', () => {
    const tracks: ParamTracks = {
      brightness: [pkf('a', 0, 0.2, 'hold'), pkf('b', 10, 1.8)],
      renderMode: [pkf('c', 0, 'GHOST_TRAIL', 'hold')],
    };
    const result = resolveAnimatedParams(tracks, 5, baseParams);
    expect(result.brightness).toBe(0.2);
    expect(result.renderMode).toBe('GHOST_TRAIL');
    // Untouched animatable field (no track) falls back to base.
    expect(result.contrast).toBe(baseParams.contrast);
    // Untouched non-animatable field is completely unaffected.
    expect(result.diffThreshold).toBe(baseParams.diffThreshold);
    expect(result.showId).toBe(baseParams.showId);
  });

  it('interpolates a linear numeric track relative to the given base/time', () => {
    const tracks: ParamTracks = { hue: [pkf('a', 0, -180, 'linear'), pkf('b', 10, 180)] };
    const result = resolveAnimatedParams(tracks, 5, baseParams);
