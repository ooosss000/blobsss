...
export class BlobTracker {
  ...
  public resize(w?: number, h?: number, bypassCap = false) {
    // Removed 1280px cap for maximum detail as requested
    let targetW = w || this.video.videoWidth  || 1920;
    let targetH = h || this.video.videoHeight || 1080;

    this.width = targetW; this.height = targetH;
    
    // DPI Handling for maximum sharpness
    ...
    this.proxyH = Math.max(1, Math.round(PROXY_W * aspect));
  ...
  public start() { this.isPlaying = true; this.loop(); }
